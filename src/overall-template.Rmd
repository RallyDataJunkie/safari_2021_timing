...
```{r}
#kable(multi_stage_times_wide)
multi_stage_win_gap_ordered = multi_stage_wide_gap %>% dplyr::arrange(factor(code, levels = overall_order))
...
pos_delta = function(curr, prev) {
  if (!is.na(curr) & !is.na(prev))
    posdelta_val = prev-curr #Positive is improvement, negative is loss 
  else
    posdelta_val = NA
...
multi_overall_lead_previous_long = out_table %>% pivot_longer(cols = -c(code), names_to = "stage", values_to = "position")  %>%
  ...
  mutate(prev_position = lag(position, order_by=code),
         ...
         overall_delta=pos_delta(position, prev_position))
```
